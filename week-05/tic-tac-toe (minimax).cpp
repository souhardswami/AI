...
//  Applying minimax game algorithm
pair<ll,pair<ll,ll>> minimax(char board[3][3],char marker,ll depth)
{
    counter++;
	...
	f(i,0,valid_moves.size())
	{
		...
		if(marker == AI_MARKER)
		{
			ll score = minimax(board,PLAYER_MARKER,depth + 1).F;
		...
		{
			ll score = minimax(board,AI_MARKER,depth + 1).F;
...
int main()
{
    ...
    while(isPlayable(board))
    {
        ...
        {
            cout<<"The position is filled :(, please choose again..."<<endl<<endl;
            continue;
        }
        else
        {
        ...
        pair<ll,pair<ll,ll>> ai_move = minimax(board,AI_MARKER,INITIAL_DEPTH);
